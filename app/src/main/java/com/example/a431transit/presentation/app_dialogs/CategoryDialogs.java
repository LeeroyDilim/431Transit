...
public class CategoryDialogs {
    ...
    public static void showAddCategoryDialog(Context context, Runnable onDialogDismiss) {
    ...
    public static void showRemoveCategoriesDialog(Context context, Runnable onDialogDismiss) {
        AlertDialog.Builder builder = new AlertDialog.Builder(context);
        View dialogView = LayoutInflater.from(context).inflate(R.layout.multi_choice_dialog, null);
        builder.setView(dialogView);
        AlertDialog dialog = builder.create();

        List<String> editableCategories = CategoryHandler.getUserCreatedCategories();
        ...
        submitButton.setOnClickListener(v -> {
            for (int i = 0; i < editableCategories.size(); i++) {
                if (checkedItems[i]) {
                    CategoryHandler.removeCategory(editableCategories.get(i));
                }
            }
    ...
    public static void showAddOrRemoveFromCategoriesDialog(Context context, BusStop busStop) {
        ...
        submitButton.setOnClickListener(v -> {
            ...
            for (int i = 0; i < editableCategories.size(); i++) {
                if (checkedItems[i]) {
                    CategoryHandler.addStopToCategory(editableCategories.get(i), busStop);
                } else {
                    CategoryHandler.removeStopFromCategory(editableCategories.get(i), busStop);
                }
            }
